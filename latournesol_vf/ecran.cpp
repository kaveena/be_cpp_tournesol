...
void lcd::print_msg(std::string msg_l1, std::string msg_l2){
  ...
  for (std::string::size_type i = 0; i< 16; ++i){
    if (i<msg_l1.size()){
      i2cData(m_i2c_lcd_control,msg_l1[i]);
    }
    else {
  ...
  i2Cmd (m_i2c_lcd_control, offset);
  usleep(1000);
  for (std::string::size_type i = 0; i < msg_l2.size(); ++i) {
    i2cData (m_i2c_lcd_control, msg_l2[i]);
  }
