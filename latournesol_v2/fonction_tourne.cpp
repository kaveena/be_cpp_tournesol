...
int suivre_lum(tournesol & mysunflower){
  ...
  if (sortie<0){
    ...
    sortie = 180 - (2*ANGLE_MIN); //on effectue une saturation pour eviter que le prototype ne s'ecrase contre la table
  }
  mysunflower.set_pos_cap(sortie);//On positionne le capteur de milieu au point d'intensite max
